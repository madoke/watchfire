...
func (m *Manager) CreateProject(opts CreateOptions) (ProjectWithEntry, error) {
	...
	// Write default secrets/instructions.md if it doesn't exist
	secretsFile := config.ProjectSecretsInstructionsFile(opts.Path)
	if _, err := os.Stat(secretsFile); os.IsNotExist(err) {
		_ = os.WriteFile(secretsFile, []byte(defaultSecretsInstructions), 0o644)
	}
...
const defaultSecretsInstructions = `# Secrets & Setup Instructions
